...
final class InspectionLocalDataSourceImpl
    implements IInspectionLocalDataSource {
  InspectionLocalDataSourceImpl({required ApiaryDatabase database})
  ...
  @override
  Future<List<Inspection>> getPendingSyncInspectionsForHive(
    String hiveId,
  ) async {
    final db = await _db;
    final rows = await db.query(
      'inspections',
      where:
          '(hive_local_id = ? OR hive_server_id = ?) AND sync_status IN (?, ?, ?)',
      whereArgs: [
        hiveId,
        hiveId,
        SyncStatus.pendingCreate.name,
  ...
  Future<void> markSynced({
    ...
    await db.update(
      'inspections',
      {'server_id': serverId, 'sync_status': SyncStatus.synced.name},
      where: 'local_id = ?',
      whereArgs: [localId],
    );
  }
  ...
  static Inspection _rowToInspection(Map<String, dynamic> row) {
    ...
    final type = InspectionType.values.firstWhere(
  ...
  static Map<String, dynamic> _inspectionToRow(Inspection inspection) {
    ...
    final serverId =
        inspection.serverId ??
        (inspection.syncStatus == SyncStatus.synced ? inspection.id : null);
    final hiveServerId =
        inspection.hiveServerId ??
        (inspection.hiveLocalId == null ? inspection.hiveId : null);
    return {
      'local_id': localId,
      'server_id': serverId,
      'hive_local_id': inspection.hiveLocalId,
