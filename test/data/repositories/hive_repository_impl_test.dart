...
PaginatedResponse<HiveResponse> _paginated(
  List<HiveResponse> items, {
...
void main() {
  ...
  setUpAll(() {
    ...
    List<HiveResponse> mutateFallback(List<HiveResponse>? current) =>
        <HiveResponse>[];
    ...
    when(
      ...
      final mutate =
          invocation.namedArguments[#mutate]
              as List<HiveResponse> Function(List<HiveResponse>?);
      mutate(null);
    });
  });
  ...
  group('getHives', () {
    ...
    test(
      'first page (page 1) replaces the cache and returns items filtered to this apiary with hasNext',
      () async {
        when(
    ...
    test(
      'the global (unfiltered) fetched page still filters the returned items down to this apiary',
      () async {
        ...
        when(() => dataSource.getHives(any(that: _pageRequest(1)))).thenAnswer(
          (_) async =>
              _paginated([hiveResponse, otherApiaryHive], hasNext: false),
        );
        ...
        result.fold(
          ...
          (page) => expect(page.items.map((hive) => hive.id), ['hive-1']),
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        ).thenAnswer((_) async => [hiveResponse]);
        when(() => dataSource.getHives(any(that: _pageRequest(2)))).thenAnswer(
    ...
    test(
      ...
      () async {
        ...
        result.fold(
          (_) => fail('expected Right'),
          (page) => expect(page.items.single.name, 'Hive 1'),
        );
      },
    );
    ...
    test('fails when offline with nothing cached for this apiary', () async {
      when(() => connectivity.isOnline).thenAnswer((_) async => false);
      final otherApiaryHive = HiveResponse(
        id: 'hive-99',
        apiaryId: 'apiary-2',
      ...
      when(
        () => localDataSource.read(),
      ).thenAnswer((_) async => [otherApiaryHive]);
    ...
    test(
      'keeps a not-yet-synced local hive alongside a fresh page-1 server list',
      () async {
        ...
        result.fold((_) => fail('expected Right'), (page) {
          expect(
            page.items.map((hive) => hive.id),
            containsAll(['hive-1', 'local-pending-1']),
          );
  ...
  group('createHive', () {
    test(
      'sends the request with the apiary id and returns the mapped hive',
      () async {
        when(
        ...
        result.fold(
          (_) => fail('expected Right'),
          (hive) => expect(hive.name, 'Hive 1'),
        );
        final captured =
            verify(
                  () => dataSource.createHive(captureAny(), apiaryId: apiaryId),
                ).captured.single
                as HiveRequest;
    ...
    test(
      'creates locally and enqueues a pending operation carrying the apiary id when offline',
      () async {
        when(() => connectivity.isOnline).thenAnswer((_) async => false);
        ...
        verifyNever(
          () => dataSource.createHive(any(), apiaryId: any(named: 'apiaryId')),
        );
    ...
    test(
      ...
      () async {
        ...
        result.fold(
          (_) => fail('expected Right'),
          (hive) => expect(hive.syncStatus, HiveSyncStatus.pending),
        );
      },
    );
  });
  ...
  group('updateHive', () {
    test(
      'sends the request and returns the mapped hive, without an apiary id in the request',
      () async {
        when(
