...
void main() {
  ...
  final serverItem = ApiaryResponse(
    id: 'apiary-1',
    name: 'Back Garden',
  ...
  group('toEntities', () {
    test(
      'tags a pending id as ApiarySyncStatus.pending and everything else as synced',
      () {
    ...
    test(
      'tags an apiary as pending when only a photo attached to it is unsynced, no apiary op at all',
      () {
        final entities = merger.toEntities(
          [serverItem],
          [imageAddOp('apiary-1')],
    ...
    test(
      'tags an apiary as failed when a photo attached to it failed to sync',
      () {
        final entities = merger.toEntities(
          [serverItem],
    ...
    test(
      'leaves an apiary synced once its photo operation itself is synced',
      () {
        final entities = merger.toEntities(
          [serverItem],
    ...
    test(
      'a photo queued for a different apiary does not mark this one as pending',
      () {
        final entities = merger.toEntities(
          [serverItem],
          [imageAddOp('some-other-apiary')],
        );
        expect(entities.single.syncStatus, ApiarySyncStatus.synced);
      },
    );
    ...
    test(
      'an apiary with its own synced create but an unsynced photo still shows pending',
      () {
        final entities = merger.toEntities(
          [serverItem],
          [
            pendingOp('apiary-1', status: OperationStatus.synced),
            imageAddOp('apiary-1'),
          ],
        );
        expect(entities.single.syncStatus, ApiarySyncStatus.pending);
