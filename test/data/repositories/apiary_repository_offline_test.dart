...
void main() {
  ...
  group('ApiaryRepositoryImpl - Online operations', () {
    ...
    test('online create calls remote API and caches to local SQLite', () async {
      when(
        () => remoteDataSource.createApiary(any()),
      ).thenAnswer((_) async => sampleResponse);
      when(
        () => localDataSource.saveServerApiaries(any()),
      ).thenAnswer((_) async {});
    ...
    test(
      'online getApiaries fetches remote, caches to SQLite and includes pendingCreates on page 1',
      () async {
        ...
        when(
          () => localDataSource.getPendingSyncApiaries(),
    ...
    test(
      'online getApiary returns local pendingUpdate version instead of server version',
      () async {
        final pendingUpdateApiary = Apiary(
          id: 'server-1',
          localId: 'local-server-1',
    ...
    test(
      'online getApiaries falls back to SQLite cache if remote fails',
      () async {
        ...
        when(
          () => localDataSource.getActiveApiaries(page: 1, limit: 20),
    ...
    test(
      ...
      () async {
        ...
        when(
          () => remoteDataSource.deleteApiary('server-1'),
        ).thenAnswer((_) async {});
        when(
          () => localDataSource.deleteApiaryPermanently('server-1'),
        ).thenAnswer((_) async {});
  ...
  group('ApiaryRepositoryImpl - Offline operations', () {
    ...
    test(
      ...
      () async {
        when(() => localDataSource.insertApiary(any())).thenAnswer(
          (invocation) async => invocation.positionalArguments[0] as Apiary,
        );
    ...
    test(
      ...
      () async {
        ...
        when(() => localDataSource.updateApiary(any())).thenAnswer(
          (invocation) async => invocation.positionalArguments[0] as Apiary,
        );
    ...
    test(
      ...
      () async {
        ...
        when(
          () => localDataSource.updateApiary(any()),
        ...
        final captured = verify(
          () => localDataSource.updateApiary(captureAny()),
        ).captured;
        ...
        expect(
          ...
          reason:
              'Adding a photo offline should mark the apiary as pendingUpdate',
        );
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          reason:
              'A pendingCreate apiary should remain pendingCreate after an offline image add',
        );
      },
    );
    ...
    test(
      ...
      () async {
        final apiaryWithImage = sampleSyncedApiary.copyWith(
