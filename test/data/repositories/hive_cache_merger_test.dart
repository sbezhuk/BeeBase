...
void main() {
  ...
  group('toEntities', () {
    ...
    test(
      'tags a hive as pending when only a photo attached to it is unsynced, no hive op at all',
      () {
        final entities = merger.toEntities(
          [serverItem],
          [imageAddOp('hive-1')],
    ...
    test(
      'tags a hive as failed when a photo attached to it failed to sync',
      () {
        final entities = merger.toEntities(
          [serverItem],
          [imageAddOp('hive-1', status: OperationStatus.failed)],
        );
    ...
    test('leaves a hive synced once its photo operation itself is synced', () {
      final entities = merger.toEntities(
        [serverItem],
        [imageAddOp('hive-1', status: OperationStatus.synced)],
      );
      expect(entities.single.syncStatus, HiveSyncStatus.synced);
    });

    test(
      'a photo queued for a different hive does not mark this one as pending',
      () {
        final entities = merger.toEntities(
          [serverItem],
          [imageAddOp('some-other-hive')],
        );
        expect(entities.single.syncStatus, HiveSyncStatus.synced);
      },
    );
    ...
    test(
      'a hive with its own synced create but an unsynced photo still shows pending',
      () {
        final entities = merger.toEntities(
          [serverItem],
          [
            pendingOp('hive-1', status: OperationStatus.synced),
            imageAddOp('hive-1'),
          ],
        );
        expect(entities.single.syncStatus, HiveSyncStatus.pending);
