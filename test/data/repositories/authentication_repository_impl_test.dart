...
void main() {
  ...
  group('register', () {
    test(
      'stores the access token and returns the mapped user on success',
      () async {
        when(
          () => dataSource.register(
        ...
        result.fold(
          ...
          (user) => expect(user.email, 'bee@example.com'),
        );
    ...
    test('maps a 409 email_taken exception to a ServerFailure', () async {
      ...
      result.fold(
        (failure) => expect(
          failure,
          isA<ServerFailure>().having((f) => f.code, 'code', 'email_taken'),
        ),
  ...
  group('login', () {
    ...
    test(
      'maps invalid_credentials to a ServerFailure without storing a token',
      () async {
        when(
          () => dataSource.login(
            email: any(named: 'email'),
  ...
  group('restoreSession', () {
    test(
      'fails fast without a network call when no token was ever stored',
      () async {
        when(
    ...
    test(
      'propagates a failure if the stored token could not be validated even after refresh',
      () async {
        when(() => tokenStorage.hasAccessToken()).thenAnswer((_) async => true);
